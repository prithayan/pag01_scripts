...
#    read_init_word_class();
    ...
    foreach my $filename (glob("$dirname/*.data_defuse_dump")) {
...
#    my $filename = 'data.txt';
        ...
        if (open(my $fh, '<:encoding(UTF-8)', $filename)) {
            while (my $row = <$fh>) {
                ...
                my $line = $row;
                my $underscore = "_";
                my $count_underscore = () = $line =~ /$underscore/g; 
                #my @tokens = split /_/,$line;
                if ($count_underscore < 3 ) {#very small defuse graph
                    next;
                }
                #print "\n $row";
                $fileName_2_nodes_map{$filename}->{$row} = 1;
            }
        } else {
...
#            my $intersection = scalar( grep $tokens_in_this_file{$_}, keys %tokens_in_other_file);
            ...
            scalar(keys %tokens_in_this_file) : scalar(keys %tokens_in_other_file));
